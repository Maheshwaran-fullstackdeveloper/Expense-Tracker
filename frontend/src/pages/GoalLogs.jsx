...
const GoalLogs = () => {
  ...
  const savingsLogs = transactions.filter((t) => t.category === "Savings");

  const filteredLogs = savingsLogs
    .filter((t) => {
        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            <h1 className="text-3xl font-bold text-primary-900 dark:text-white tracking-tight">
              Savings{" "}
              <span className="text-primary-500 font-bold">logbook</span>
            </h1>
            <p className="text-primary-400 dark:text-primary-500 font-bold text-sm">
              Your path to financial freedom, one entry at a time.
            </p>
          </div>
        </div>
      </header>
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="relative group">
                  <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-primary-300 group-focus-within:text-primary-500 transition-colors" />
                  <input
                    type="text"
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                    placeholder="Search notes..."
                    className="w-full h-12 pl-11 pr-4 bg-primary-50/50 dark:bg-slate-800/50 border-2 border-transparent focus:border-primary-500 rounded-2xl outline-none transition-all font-semibold text-primary-900 dark:text-white placeholder:text-primary-200 dark:placeholder:text-primary-800"
                  />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full h-12 px-4 bg-primary-50/50 dark:bg-slate-800/50 border-2 border-transparent focus:border-primary-500 rounded-2xl outline-none transition-all font-bold text-primary-900 dark:text-white cursor-pointer"
                />
              </div>
              ...
              <button
                ...
                className="w-full py-4 text-xs font-bold text-primary-400 dark:text-primary-600 hover:text-primary-600 dark:hover:text-primary-400 transition-all underline cursor-pointer"
              >
                Reset All Filters
              </button>
            </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead className="border-b border-primary-50 dark:border-primary-900/30">
                  <tr>
                    ...
                    <th className="px-8 py-5 text-xs font-bold text-primary-400 uppercase tracking-widest text-right">
                      Amount
                    </th>
                  </tr>
                </thead>
                    ...
                    >
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-4">
                          <div className="w-10 h-10 bg-primary-50 dark:bg-slate-800 rounded-xl flex items-center justify-center text-primary-500 dark:text-primary-400 group-hover:bg-primary-500 group-hover:text-white transition-all shadow-sm">
                            <Coins className="w-5 h-5" />
                          </div>
                          <div>
                            <p className="text-primary-900 dark:text-white font-bold text-sm capitalize">
                              {log.notes || "Savings Deposit"}
                            </p>
                            <p className="text-sky-500 dark:text-sky-400 text-[10px] font-bold uppercase tracking-tighter">
              ...
              {filteredLogs.length === 0 && (
                <div className="py-32 text-center">
                  ...
                  <p className="text-primary-400 dark:text-primary-500 text-sm font-medium">
                    Try adjusting your range or search terms
                  </p>
                </div>
              )}
            </div>
                      ...
                      if (
                        ...
                        return (
                          <button
                            ...
                            className={`w-9 h-9 rounded-xl text-xs font-bold transition-all ${currentPage === pageNum ? "bg-primary-500 text-white shadow-lg shadow-primary-500/20" : "hover:bg-white dark:hover:bg-slate-800 text-primary-400"}`}
                          >
                            {pageNum}
                          </button>
                        );
